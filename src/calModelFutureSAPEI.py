# -*- coding: utf-8 -*-
"""
Created on Wed Jun 25 19:08:12 2025
...
dt =  pd.date_range(start='2015-01-01', end='2100-12-31', freq='D')

#MODEL = ['MRI-ESM2-0']#'GFDL-ESM4','IPSL-CM6A-LR','MPI-ESM1-2-HR','MRI-ESM2-0'
#MODEL = ['IPSL-CM6A-LR']
#MODEL = ['MPI-ESM1-2-HR']
MODEL = ['GFDL-ESM4']
...
for model in MODEL:
    for period1 in Period1:
        for period2 in Period2:
            ...
            e = np.array([5000,10000,15000,20000,25000,30000,sha[0]],dtype = int)
            ...
            for timep in tp:
                ...
                sapei = SAPEI[(dt.year>=ts) &(dt.year<=te),:]
                sapeif = np.float32(np.zeros([len(sapei),210,341])*np.nan)
                for i in range(len(sapei)):
                    sapeif[i,mask==1] = sapei[i,:]
