...
p1.append(pop)
p2.append(Pop_H[y]['Rural'])
p3.append(Pop_H[y]['Urban'])
pxxx = Pop_H[y]['Urban']/pop
pxxx[Pop_H[y]['Urban']>=pop] =1
p4.append(pxxx)
f.append(CDHD1s[Years3==y])
e.append(pop*CDHD1s[Years3==y])
pa = np.array(p1)
pb = np.array(p2)
po = np.array(p3)
pv = np.array(p4)
...
Data['historical'] ={'pop_all':np.squeeze(pa)*mask,
                     ...
                     'pop_v':np.squeeze(pv)*mask,
                     'e':np.squeeze(pe)*mask,
                     'h':np.squeeze(np.array(f))*mask
                     }
...
for s in range(1,4):
    ...
    for i in range(len(Years)):
        pop = np.squeeze(Pop_F[SN1[s]]['Total'][i])
        ...
        p1.append(pop)
        p2.append(np.squeeze(Pop_F[SN1[s]]['Rural'][i]))
        p3.append(np.squeeze(Pop_F[SN1[s]]['Urban'][i]))
        pxxx = np.squeeze(Pop_F[SN1[s]]['Urban'][i])/ pop
        
        pxxx[np.squeeze(Pop_F[SN1[s]]['Urban'][i])>=pop]= 1
        p4.append(pxxx)
        
        f.append(CDHD2s[SN2[s]][fy==Years[i]])
        e.append(pop*CDHD2s[SN2[s]][fy==Years[i]])
    pa = np.array(p1)
    pb = np.array(p2)
    po = np.array(p3)
    pe = np.array(e)
...
for s in ['SSP1','SSP3','SSP5']:
    E_b = Data['historical']['e']
    E_s = np.nanmean(Data[s]['e'][rg],axis=0)
    
    pop_b = Data['historical']['pop_all']
    pop_s = np.nanmean(Data[s]['pop_all'][rg],axis=0)
    
    vp_b = Data['historical']['pop_v']
    vp_s = np.nanmean(Data[s]['pop_v'][rg],axis=0)
    
    h_b = Data['historical']['h']
    h_s = np.nanmean(Data[s]['h'][rg],axis=0)
    ...
    for a in range(7):
        if a<6:
            ...
            B[Ar[a]] = np.array([rh,rp,rv,rh+rp+rv])
        else:
            ca = ag.copy()
            ...
            rv = np.nansum(w*ca*d_v)
            rp = np.nansum(w*ca*d_p)
            B['Asia'] = np.array([rh,rp,rv,rh+rp+rv])
    A[s] = B
...
# 将图例添加到整个图形（而非单个子图）
fig.legend(
    [h[0],h[2],h[4]], ['Cimate change','Pop.growth','pop fraction'],
    loc='lower center',  # 位置：底部中央
    ncol=3,  # 图例列数
    bbox_to_anchor=(0.5, -0.02),  # 微调位置（相对于图形）
    fontsize=15,
