# -*- coding: utf-8 -*-
"""
Created on Sat Jun 21 12:47:05 2025
...
with open('../data/BR_DEM.pkl', 'rb') as f:  # 读取pickle文件
    alt = joblib.load(f)
    f.close()
...
MODEL = ['MPI-ESM1-2-HR']#,'IPSL-CM6A-LR','MPI-ESM1-2-HR','MRI-ESM2-0']
        ...
        for period1 in Period1:
            for period2 in Period2:
                ...
                if not os.path.exists(d1+'/'+d2):
                    os.makedirs(d1+'/'+d2) 
                if not os.path.isfile(d1+'/'+d2+'/'+period2+'_pet_pr_daily_'+timep+'.nc'):
