...
fy3 = np.arange(2020,2101,10)
for s in range(3):

    gdfd = zonal_statistics(
            shp_path='H:/65国家/ASIA/Asia_Sub.shp',
            matrix=np.nanmean(CDHD2s[sn2[s]][(fy>=2020)&(fy<=2040),:,:],axis=0),
            lat = lat,
            lon = lon,
            stats=['sum', 'mean'] )
    Pop = []
    for i in range(2050,2071,10):
        
        Pop.append(Pop_F[sn1[s]][urt][fy3==i] )
    Pop = np.array(Pop)  
...
#pc.set_facecolor(np.array([0.1,0.2,0.3,0.3]))
    ...
    ax2.set_xlabel('pop,million',labelpad = 0.08,fontsize = 7)
    ax2.set_ylabel('CDHD,days',labelpad = 0.08,fontsize = 7)
