...
p1.append(mask*poph*fbh)
p2.append(mask*poph*fsh)
p3.append(mask*poph*fwh)
p4.append(mask*poph*foh)
 
pb = np.array(p1)
ps = np.array(p2)
pw = np.array(p3)
po = np.array(p4)
pa = np.squeeze(pb)+ np.squeeze(ps)+ np.squeeze(pw)+ np.squeeze(po)
     

Data['historical'] ={'pop_all':np.squeeze(pa)*mask,
                'pop_old':np.squeeze(po)*mask,
                'pop_baby':np.squeeze(pb)*mask,
                'pop_s':np.squeeze(ps)*mask,
                'pop_w':np.squeeze(pw)*mask}
...
for s in range(1,4):
    ...
    for i in range(len(Years)):
        ...
        p1.append(mask*pop*fb)
        p2.append(mask*pop*fs)
        p3.append(mask*pop*fw)
        p4.append(mask*pop*fo)
    ...
    Data[SN1[s]] = {'pop_all':np.squeeze(pa)*mask,
                    'pop_old':np.squeeze(po)*mask,
                    'pop_baby':np.squeeze(pb)*mask,
                    'pop_s':np.squeeze(ps)*mask,
                    'pop_w':np.squeeze(pw)*mask}
...
for s in ['historical'] :
    ...
    for a in range(7):
        if a<6:
            ...
            pba = np.nansum(np.nansum(ca*PBA,axis=1 ),axis=0 )
            pbb = np.nansum(np.nansum(ca*PBB,axis=1 ),axis=0 )
            pbo = np.nansum(np.nansum(ca*PBO,axis=1 ),axis=0 )
            pbs = np.nansum(np.nansum(ca*PBS,axis=1 ),axis=0 )
            pbw = np.nansum(np.nansum(ca*PBW,axis=1 ),axis=0 )
            B[regions[a]] = np.array([pbb,pbs,pbw,pbo,pba])
        else:
            ca = ag.copy()
            ...
            B['Asia'] =  np.array([pbb,pbs,pbw,pbo,pba])
    AH[s] = B
...
for s in ['SSP1','SSP3','SSP5']:
    ...
    for a in range(7):
        if a<6:
            ...
            pbs = np.nansum(np.nansum(ca*PBS,axis=1 ),axis=1 )
            pbw = np.nansum(np.nansum(ca*PBW,axis=1 ),axis=1 )
            B[regions[a]] = np.array([pbb,pbs,pbw,pbo,pba])
        else:
            ca = ag.copy()
...
ZT = ['preschool children','students','worker','elder','Total',]
for z in range(5):
    fig = plt.figure(figsize=(15, 4))
    fig.suptitle(urt+' '+ZT[z], fontsize=16, y=1.1)
    plt.rcParams.update({
        'axes.grid': False,
        'grid.alpha': 0.3,
        'axes.labelsize': 13,
    ...
    for i in range(6):
        ...
        d1 = np.insert(d1,0,AH['historical'][all_regions[i]][z]/1e6)
        d2 = A['SSP3'][all_regions[i]][z]/1e6
        d2 = np.insert(d2,0,AH['historical'][all_regions[i]][z]/1e6)
        d3 = A['SSP5'][all_regions[i]][z]/1e6
        d3 = np.insert(d3,0,AH['historical'][all_regions[i]][z]/1e6)
        ssp_cat  = ['126', '370', '585']
        X = np.insert(Years,0,2000)
        plt.plot(X,d1,color = colors[0])
        plt.plot(X,d2,color = colors[1])
        plt.plot(X,d3,color = colors[2])
        print(Ar[i])
