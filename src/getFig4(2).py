...
SN1 = ['historical','SSP1','SSP3','SSP5']#['historical','SSP1','SSP3','SSP5']
...
def getData(D,mo):
    ...
    Data['historical'] ={'pop_all':np.squeeze(pa*mask),'pop_old':np.squeeze(po*mask),'pop_baby':np.squeeze(pb*mask),
                         'e':np.squeeze(pe*mask),'f':np.squeeze(np.array(f)*mask),'d':np.squeeze(np.array(d)*mask),'i':np.squeeze(np.array(ss)*mask)
                         }
    ...
    for s in range(1,4):
        ...
        Data[SN1[s]] = {'pop_all':np.squeeze(pa)*mask,'pop_old':np.squeeze(po)*mask,'pop_baby':np.squeeze(pb)*mask,
                        'e':np.squeeze(pe)*mask,'f':np.squeeze(np.array(f)*mask),'d':np.squeeze(np.array(d)*mask),'i':np.squeeze(np.array(ss)*mask)
...
def getfill(DATA,s,v,timep):
    ...
    for m in MODEL:
        ...
        if s == 'historical':
            ...
            #y = y0*x1
            y = np.squeeze(np.nanmean(Data[s]['pop_all'][timep],axis=0))
        x = x[y==y]
        y = y[y==y]
...
Data = getData(CDHEs_data,'MME')
for a in  range(3): 
    k = 0
    lines = []
    fills = []                                                                                    
    for s in ['historical','SSP1','SSP3','SSP5']:
        xf,ub,lb,ym = getfill(CDHEs_data,s,va[a],timep)
        if s == 'historical':
                x = np.squeeze(Data[s][va[a]])
                #x0 = Data[s][va[a]][-1]
                #x1 = x0.copy()
                #x1[x1>0] = 1
                #x1[x1!=1] = 0
        ...
        #     print((s+' '+str(np.nanmax(ym[-1]/1e9)-ym[xf<=90][-1]/1e9)))
        # if va[a]=='f':
        
        #     print((s+' '+str(np.nanmax(ym[-1]/1e9)-ym[xf<=2][-1]/1e9)))
        # if va[a]=='i':
        
        #     print((s+' '+str(np.nanmax(ym[-1]/1e9)-ym[xf<=120][-1]/1e9)))
            
        
        if va[a]=='d':
        
            print(s+' '+str(1- (ym[xf<=90][-1])/ym[-1]))
        if va[a]=='f':
        
            print(s+' '+str(1-(ym[xf<=2][-1])/ym[-1]))
        if va[a]=='i':
        
            print(s+' '+str(1-(ym[xf<=120][-1])/ym[-1]))
    ...
    axs[a].set_ylabel('population,billion' )
