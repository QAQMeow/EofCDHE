# -*- coding: utf-8 -*-
"""
Created on Fri Jul 25 19:37:15 2025
...
Areas = mask*calculate_grid_area(maskdata['LAT'])
...
def PLot1(ax,mlon,mlat,data,i,j,c,vmax,xl,yl):
    ...
    if vmax >100:
        bounds = np.arange(0,vmax,np.round(np.linspace(0, vmax,c.N-1)/10)[1]*10)
        bounds = np.insert(bounds,0,np.round(np.nanmin(data)/10)*10)
    else:
        bounds = np.round(np.linspace(0, vmax,c.N-1),2)
    ...
    #print(bounds)
    #print(100*counts/np.nansum(counts))
    ...
    if xl == 'change in annual CDHD':
        ...
        da = da1*Areas
        print(100*np.nansum(da)/np.nansum(Areas))
    
    if xl == 'change in annual CDHF':
        ts = 2
...
gs = gridspec.GridSpec(3, 3, height_ratios=[1, 1, 1,], hspace=0.1)
...
van = ['change in annual CDHD','change in annual CDHF','change in annual CDHI']

cl = [ cmaps.MPL_YlOrBr[np.arange(2,128,12)], cmaps.MPL_Oranges[np.arange(0,128,12)],cmaps. MPL_gnuplot2_r[np.arange(2,128,12)]]
...
for i in range(3):
    for j in range(3):
        ...
        detla_d = d2-d1
        print(sn[i])
        p = PLot1(ax,lon,lat,detla_d,i,j,cl[j],vmax,van[j],sn[i])
        if i==0:
            barls.append(p)


for j in range(3):            
    cbar_ax = fig.add_axes([0.05+j*0.34, 0.02, 0.23, 0.02])

    cbar = fig.colorbar(barls[j], cax=cbar_ax, orientation='horizontal',extend='both',shrink=0.2, aspect=40,
